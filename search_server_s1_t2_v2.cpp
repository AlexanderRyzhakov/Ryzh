//============================================================================
// Name        : search_server_s1_t2_v2.cpp
// Author      : –ыжаков јлександр
// Version     : 1.0
// Copyright   : Your copyright notice
// Description : 1 спринт
//============================================================================
...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query, KeyMapper keymapper) const {
        ...
        vector<Document> matched_documents;
        matched_documents = FindTopDocuments(raw_query, [](int document_id, DocumentStatus status, int rating)
                                                                {return status == DocumentStatus::ACTUAL;});
            return matched_documents;
        }
    ...
    vector<Document> FindAllDocuments(const Query& query, KeyMapper keymapper) const {
        ...
        for (const string& word : query.plus_words) {
            ...
            for (const auto [document_id, term_freq] : word_to_document_freqs_.at(word)) {
                if (keymapper(document_id, documents_.at(document_id).status, documents_.at(document_id).rating) == 1) {
